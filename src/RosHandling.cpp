...
RosHandling::RosHandling(System *pSys, LocalMapping *pLocal, rclcpp::Node::SharedPtr pMainNode)
	: mp_system(pSys), mp_LocalMapping(pLocal), mpMainNode(pMainNode)
{
	image_transport::ImageTransport it(mpMainNode);

	// To avoid having to create a new publisher every time RosHandling::BroadcastTF is called
	mp_tf_broadcaster = std::make_shared<tf2_ros::TransformBroadcaster>(mpMainNode);

	mp_img_l_pub = it.advertise("/AQUA_SLAM/left/image_raw", 10);

	mp_img_r_pub = it.advertise("/AQUA_SLAM/right/image_raw", 10);

	mp_img_info_pub = it.advertise("/AQUA_SLAM/img_with_info", 10);

	mp_img_merge_cond_pub = it.advertise("/AQUA_SLAM/img_merge_cand", 10);

	mp_integration_path_pub = mpMainNode->create_publisher<nav_msgs::msg::Path>("/AQUA_SLAM/integration_path", 10);
	
    mp_ref_integration_path_pub = mpMainNode->create_publisher<nav_msgs::msg::Path>("/AQUA_SLAM/ref_integration_path", 10);
    
    // initialize mp_markers_pub
    mp_markers_pub = mpMainNode->create_publisher<visualization_msgs::msg::MarkerArray>("/AQUA_SLAM/markers", 10);
    
	mp_pose_orb_pub = mpMainNode->create_publisher<geometry_msgs::msg::PoseStamped>("/AQUA_SLAM/orb_pose", 10);

	mp_odom_orb_pub = mpMainNode->create_publisher<nav_msgs::msg::Odometry>("/AQUA_SLAM/orb_odom", 10);
	
	mp_path_orb_pub = mpMainNode->create_publisher<nav_msgs::msg::Path>("/AQUA_SLAM/orb_path", 10);
	
	mp_pose_orb_camera_pub = mpMainNode->create_publisher<nav_msgs::msg::Odometry>("/AQUA_SLAM/camera_pose", 10);

	mp_path_ekf_pub = mpMainNode->create_publisher<nav_msgs::msg::Path>("/AQUA_SLAM/ekf_path", 10);  // doesnt seem to ever publish

	mp_pointcloud_pub = mpMainNode->create_publisher<sensor_msgs::msg::PointCloud2>("/AQUA_SLAM/sparse_map", 100);

	mp_octomap_pub = mpMainNode->create_publisher<octomap_msgs::msg::Octomap>("/AQUA_SLAM/octomap", 10);
	...
	mp_pose_integration_ref_pub = mpMainNode->create_publisher<geometry_msgs::msg::PoseStamped>("/AQUA_SLAM/integration_ref", 10);

	mp_pose_integration_cur_pub = mpMainNode->create_publisher<geometry_msgs::msg::PoseStamped>("/AQUA_SLAM/integration_cur", 10);
	
	mp_save_srv = mpMainNode->create_service<std_srvs::srv::Empty>(
					"/AQUA_SLAM/save",
					std::bind(&RosHandling::SavePose,
						this,
						std::placeholders::_1,
						std::placeholders::_2
					)
	...
	m_load_srv = mpMainNode->create_service<std_srvs::srv::Empty>(
						"/AQUA_SLAM/load_map",
						std::bind(&RosHandling::LoadMap,
							this,
							std::placeholders::_1,
							std::placeholders::_2
						)
	...
	m_calib_srv = mpMainNode->create_service<std_srvs::srv::Empty>(
						"/AQUA_SLAM/calibrate",
						std::bind(&RosHandling::CalibrateDVLGyro,
							this,
							std::placeholders::_1,
							std::placeholders::_2
						)
                              ...
                              const string &child_id)
{
	...
	mp_tf_broadcaster->sendTransform(tf_msg);
}
...
void RosHandling::PublishLossKF(set<KeyFrame*, KFComparator> &loss_kfs)
{
    ...
    RCLCPP_DEBUG_STREAM(mpMainNode->get_logger(), "R_b0_w: " << R_b0_w);
...
bool RosHandling::SavePose(const std::shared_ptr<std_srvs::srv::Empty::Request> req, 
...
bool RosHandling::LoadMap(const std::shared_ptr<std_srvs::srv::Empty::Request> req, 
						  std::shared_ptr<std_srvs::srv::Empty::Response> res)
{
	string map_file;
	// default declaration (only in case uninitialised)
	mpMainNode->declare_parameter("map_file", "");  
	mpMainNode->get_parameter("map_file", map_file);
...
bool RosHandling::FullBA(const std::shared_ptr<std_srvs::srv::Empty::Request> req,
              			 std::shared_ptr<std_srvs::srv::Empty::Response> res)
{
