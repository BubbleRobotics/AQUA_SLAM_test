...
System::System(const string &strVocFile, const string &strSettingsFile, const eSensor sensor, rclcpp::Node::SharedPtr pMainNode,
               const bool bUseViewer, const int initFr, const string &strSequence, const string &strLoadingFile)
	: mSensor(sensor), mpMainNode(pMainNode), mbReset(false), mbResetActiveMap(false),
	  mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mStrVocabularyFilePath(strVocFile)
{
	...
	mpAtlas = new Atlas(0);
	bool bLoadMap = false;
	// declare and get param (declare only provides default if not declared before)
	mpMainNode->declare_parameter("/ORBSLAM3_tightly/is_load_map", false);
	mpMainNode->get_parameter("/ORBSLAM3_tightly/is_load_map", bLoadMap);
	string out_path;
	mpMainNode->declare_parameter("/ORBSLAM3_tightly/out_path", "");
	mpMainNode->get_parameter("/ORBSLAM3_tightly/out_path", out_path);
...
else
{
	...
	//set DenseMapper
	mpDenseMapper = new DenseMapper(strSettingsFile, mpMainNode);
	auto dense_mapping = new thread(&ORB_SLAM3::DenseMapper::Run, mpDenseMapper);

	mRosHandler = new RosHandling(this, mpLocalMapper, mpMainNode);
...
void System::dvlCallBack(const nav_msgs::msg::Odometry &dvl)
{
                                  ...
                                  string filename)
{
    ...
    try {
        ...
        RCLCPP_ERROR_STREAM(mpMainNode->get_logger(), "OpenCV Error: " << e.what());
        // Handle the exception, e.g., by returning an error code or rethrowing
        Tcw = cv::Mat::eye(4, 4, CV_32F);
    } catch (const std::exception& e) {
        // Handle standard exceptions
        RCLCPP_ERROR_STREAM(mpMainNode->get_logger(), "Error: " << e.what());
        // Handle the exception
        Tcw = cv::Mat::eye(4, 4, CV_32F);
    } catch (...) {
        // Catch all other types of exceptions
        RCLCPP_ERROR_STREAM(mpMainNode->get_logger(), "Unknown Exception");
        // Handle the exception
        Tcw = cv::Mat::eye(4, 4, CV_32F);
    }
//	cv::Mat Tcw;
...
{
	RCLCPP_INFO_STREAM(mpMainNode->get_logger(), "start to save Altlas");
