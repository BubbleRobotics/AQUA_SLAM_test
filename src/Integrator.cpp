...
{
    std::ostringstream oss;  // stringstream print with Verbos::PrintMess as a replacement for ros1 macros

    auto cur_time = cur_F.mTimeStamp;
    auto pre_time = cur_F.mpPrevFrame->mTimeStamp;
    if (pre_time == 0) {
        oss << "WARNING: " << std::fixed << std::setprecision(6) << "non prev frame, frame timestamp: " << cur_time;
        Verbose::PrintMess(oss.str(), Verbose::VERBOSITY_VERBOSE);
        cur_F.setIntegrated();
        return;
    }
    else if (DvlGyroDataQueue.size() == 0) {
        Verbose::PrintMess("WARNING: Not IMU data in mlQueueDVLGyroData!!", Verbose::VERBOSITY_VERBOSE);
        cur_F.setIntegrated();
        return;
    }
    ...
    if (n == 0) {
        Verbose::PrintMess("WARNING: no measurement to integrate!", Verbose::VERBOSITY_VERBOSE);
        return;
    }
    ...
    // KF
    if (!mpIntFromKF_C2C) {
        Verbose::PrintMess("ERROR: null poniter mpIntFromKF_C2C", Verbose::VERBOSITY_QUIET);
        assert(0);
    }
    ...
    if (mpIntFromKF_C2C->bDVL) {
        ...
        if(!isDVL_there){
            Verbose::PrintMess("WARNING: wait init, skip integration", Verbose::VERBOSITY_VERBOSE);
            return;
        }
    }
    else {
    ...
    if (mpIntFromF_C2C->dV.at<double>(0) == 0) {
        Verbose::PrintMess("ERROR: Velocity is not initiialzed!", Verbose::VERBOSITY_QUIET);
...
void Integrator::SetLossRefKF(KeyFrame* pKF)
{
    ...
    oss << std::fixed << setprecision(6) << "set Loss Ref KF[" << pKF->mnId << "] " << pKF->mTimeStamp;
