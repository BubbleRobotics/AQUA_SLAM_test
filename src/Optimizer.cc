...
void Optimizer::LocalStereoImuOnlyBA(Atlas* pAtlas, KeyFrame* pKF, bool* pbStopFlag, Map* pMap, int &num_fixedKF,
                                     double lamda_DVL, double lamda_visual)
{

	// Original signature: KeyFrame *pKF, bool *pbStopFlag, Map *pMap, bool bLarge, bool bRecInit
	// Since this is the original implementation from ORB SLAM 3, the passed parameters lamda_DVL and lamda_visual are not used

	std::chrono::steady_clock::time_point t0 = std::chrono::steady_clock::now();
	Map *pCurrentMap = pKF->GetMap();

	// (max) number of iterations of the optimiser
	int opt_it = 10;

	// Number of frames in local window; max 10 but may be less if fewer keyframes are available
	const int Nd = std::min(10, (int)pCurrentMap->KeyFramesInMap() - 2); 
	const unsigned long maxKFid = pKF->mnId;
	...
	for (int i = 0; i < N; i++) {
		...
		if (pKFi->bImu && pKFi->mPrevKF->bImu && pKFi->mpImuPreintegrated) {
			...
			// Originally, the parameter bRecInit controlled if this clause will ever trigger. By removing bRecInit
			// this is always happens for the last keyframe being optimised.
			if (i == N - 1) {
				// All inertial residuals are included without robust cost function, but not that one linking the
				// last optimizable keyframe inside of the local window and the first fixed keyframe out. The
				// information matrix for this measurement is also downweighted. This is done to avoid accumulating
