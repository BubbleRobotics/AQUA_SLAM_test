                    ...
                    if (mpAtlas->GetAllMaps().size()==1) {
                        ...
                        if(!mpAtlas->isImuInitialized()){
                            if((mpAtlas->KeyFramesInMap() > 10)){
								Verbose::PrintMess("DVL-IMU init", Verbose::VERBOSITY_NORMAL);
                                InitializeDvlIMU();
                            }
                        }
                        else if((!mpAtlas->IsIMUCalibrated())&&(dis.first>mInitTranslationThred&&dis.second>mInitRotationThred)){
							Verbose::PrintMess("try initialize IMU with sufficient motion", Verbose::VERBOSITY_NORMAL);
                            InitializeDvlIMU();
                        }
                        ...
                        if(mpAtlas->GetAllKeyFramesinAllMap().size()>200&&current_KF_num<200){
							Verbose::PrintMess("DVL-IMU refine", Verbose::VERBOSITY_NORMAL);
                            RefineGravityDvlIMU();
                            current_KF_num =  mpAtlas->GetAllKeyFramesinAllMap().size();
                        }
...
//	Optimizer::DvlGyroInitOptimization5(mpAtlas->GetCurrentMap(), mbg, mbMonocular, priorG);
    ...
    if(clib_avg_error<0.01&&(all_kf.size()>20)){
		...
		Verbose::PrintMess("init avg error<100, avg err < 0.01", Verbose::VERBOSITY_NORMAL);
        ...
        unique_lock<shared_timed_mutex> lock(pCurMap->mMutexMapUpdate);
		Verbose::PrintMess("init motion is not enough", Verbose::VERBOSITY_NORMAL);
        ResetKFBias();
        pCurMap->SetImuInitialized();
        // mpAtlas->SetDvlImuInitialized();
        mpAtlas->setRGravity(pCurMap->getRGravity());
	...
	Verbose::PrintMess("init motion is enough, but error too large", Verbose::VERBOSITY_NORMAL);
    unique_lock<shared_timed_mutex> lock(pCurMap->mMutexMapUpdate);
    pCurMap->SetImuInitialized();
    mpAtlas->SetDvlImuInitialized();
    mpAtlas->setRGravity(pCurMap->getRGravity());
...
{
	Verbose::PrintMess("reset bias", Verbose::VERBOSITY_NORMAL);
    auto all_kf = mpAtlas->GetAllKeyFrames();
    for(auto pkf:all_kf){
        IMU::Bias b;
        pkf->SetNewBias(b);
