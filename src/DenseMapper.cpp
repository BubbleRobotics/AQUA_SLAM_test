...
namespace ORB_SLAM3
{
    DenseMapper::DenseMapper(string settingFile, rclcpp::Node::SharedPtr pMainNode)
    : mpMainNode(pMainNode)
    {
        image_transport::ImageTransport it(mpMainNode);

        mDepthPub = it.advertise("/AQUA_SLAM/dense_mapper/depth", 10);
    ...
    void DenseMapper::InsertNewKF(KeyFrame* pKF)
    {
        if (pKF->GetMap()->GetAllKeyFrames().size() < 5) {
            RCLCPP_DEBUG_STREAM(mpMainNode->get_logger(), "DenserMapper: less than 5 KF in current map, skip");
            return;
        }
                                  ...
                                  DepthEstParamters &param)
    {
        ...
        if (param._correlation_window_size <= 0 || param._correlation_window_size % 2 != 1) {
            RCLCPP_ERROR_STREAM(mpMainNode->get_logger(), "DenseMapper: Incorrect window_size value: it should be positive and odd");
            return;
        }
