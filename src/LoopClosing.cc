...
LoopClosing::LoopClosing(Atlas *pAtlas, 
                         ...
                         rclcpp::Node::SharedPtr pMainNode)
  : mbResetRequested(false), mbResetActiveMapRequested(false), mbFinishRequested(false), mbFinished(true), mpAtlas(pAtlas),
    ...
    mpMainNode(pMainNode)
{
    mnCovisibilityConsistencyTh = mMergingThreshold;
    mpLastCurrentKF = static_cast<KeyFrame*>(NULL);
    image_transport::ImageTransport it(mpMainNode);
    mImgPub_cur_keyframe = it.advertise("/AQUA_SLAM/loop/cur_img",10);
    mImgPub_map_keyframe = it.advertise("AQUA_SLAM/loop/map_img",10);
	mTargetMapID = -1;
}
...
//                cout <<"BoW: " << numProjMatches << " matches between " << vpMapPoints.size() << " points with coarse Sim3" << endl;
				RCLCPP_INFO_STREAM(mpMainNode->get_logger(), "re-projection matched points: "<<numProjMatches<<" threshold: "<<nProjMatches);
                ...
                if(numProjMatches >= nProjMatches)
                {
                    ...
                    RCLCPP_INFO_STREAM(mpMainNode->get_logger(), "Optimize Sim3: "<<numOptMatches<<" inliers, threshold: "<<nSim3Inliers);
                    if(numOptMatches >= nSim3Inliers)
                    {
						...
						RCLCPP_INFO_STREAM(mpMainNode->get_logger(), numProjOptMatches << " matches after of the Sim3 optimization"<<", threshold: "<<numProjOptMatches);
...
void LoopClosing::MergeLocal()
{
    ...
    if (pCurrentMap != mpAtlas->GetCurrentMap()) {
        RCLCPP_ERROR_STREAM(mpMainNode->get_logger(),"try to merge inactive map, stop");
        mpLocalMapper->Release();
        return;
    }
...
//            Verbose::PrintMess("Bad KF in correction", Verbose::VERBOSITY_DEBUG);
			RCLCPP_ERROR_STREAM(mpMainNode->get_logger(),"Bad KF in correction");
			mpLocalMapper->Release();
			return;
        }
...
//			Verbose::PrintMess("Other map KF, this should't happen", Verbose::VERBOSITY_DEBUG);
			if(pKFi->GetMap()==nullptr||pKFi->GetMap()->IsBad())
				RCLCPP_ERROR_STREAM(mpMainNode->get_logger(),"try to merge null map KF, this should't happen");
			else
				RCLCPP_ERROR_STREAM(mpMainNode->get_logger(),"try to merge KF in map: "<<pKFi->GetMap()->GetId()<<", this should't happen");
			mpLocalMapper->Release();
			return;
		}
...

	{
        ...
        RCLCPP_DEBUG_STREAM(mpMainNode->get_logger(), "check KF size: " << vCorrectedSim3.size());
		for (auto it: vCorrectedSim3) {
			...
			if (!(it.first) || (it.first->isBad())) {
				...
				RCLCPP_ERROR_STREAM(mpMainNode->get_logger(),"Bad Map in correction");
				mpLocalMapper->Release();
				return;
			}
            RCLCPP_DEBUG_STREAM(mpMainNode->get_logger(),"KF check before map merge, KF: " << it.first->mnId << ", " << " in map: "
...
//        pMergeMap->ChangeId(pCurrentMap->GetId());
        ...
        else
        {
            ...
            RCLCPP_DEBUG_STREAM(mpMainNode->get_logger(),"MERGE-VISUAL: There are " + to_string(pMergeMap->MapPointsInMap()) + " MPs in the map");
            RCLCPP_DEBUG_STREAM(mpMainNode->get_logger(),"MERGE-VISUAL: It will be inserted " + to_string(vpCurrentMapMPs.size()) + " MPs in the map");
...
void LoopClosing::SearchAndFuse(const KeyFrameAndPose &CorrectedPosesMap, vector<MapPoint*> &vpMapPoints)
{
    ...
    for(KeyFrameAndPose::const_iterator mit=CorrectedPosesMap.begin(), mend=CorrectedPosesMap.end(); mit!=mend;mit++)
    {
		...
		if (pKFi==NULL||pMap==NULL){
			RCLCPP_ERROR_STREAM(mpMainNode->get_logger(),"fail to fuse map point because of NULL pointer");
			continue;
		}
		else if(pKFi->isBad()){
			RCLCPP_ERROR_STREAM(mpMainNode->get_logger(),"fail to fuse map points, because of bad KF");
			continue;
		}
		else if(pMap->IsBad()){
			RCLCPP_ERROR_STREAM(mpMainNode->get_logger(),"fail to fuse map points, because of bad Map");
			continue;
		}
